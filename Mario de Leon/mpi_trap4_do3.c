...
void Build_mpi_type(double *a_p, double *b_p, int *n_p, MPI_Datatype *input_mpi_t_p)
{
    // 1. Definir las longitudes de los bloques y los tipos de datos.
    int array_of_blocklengths[3] = {1, 1, 1};
    MPI_Datatype array_of_types[3] = {MPI_DOUBLE, MPI_DOUBLE, MPI_INT};

    // 2. Obtener las direcciones de memoria de las variables.
    MPI_Aint a_addr, b_addr, n_addr;
    MPI_Get_address(a_p, &a_addr);
    MPI_Get_address(b_p, &b_addr);
    MPI_Get_address(n_p, &n_addr);

    // 3. Calcular los desplazamientos entre estas direcciones.
    MPI_Aint array_of_displacements[3];
    array_of_displacements[0] = 0;
    array_of_displacements[1] = b_addr - a_addr;
    array_of_displacements[2] = n_addr - a_addr;

    // 4. Utilizar MPI_Type_create_struct para crear el tipo derivado.
    MPI_Type_create_struct(3, array_of_blocklengths, array_of_displacements, array_of_types, input_mpi_t_p);

    // 5. Finalmente, comprometer el tipo derivado con MPI_Type_commit.
...
void Get_input(int my_rank, int comm_sz, double *a_p, double *b_p, int *n_p)
{
    ...
    // Definimos una estructura para encapsular los datos
    struct
    {
        double a;
        double b;
    ...
    // Usamos un único MPI_Bcast para enviar/recibir la estructura de datos
    MPI_Bcast(&data, 1, input_mpi_t, 0, MPI_COMM_WORLD);

    // Asignamos los valores recibidos a las variables correspondientes
    *a_p = data.a;
    *b_p = data.b;
    *n_p = data.n;
