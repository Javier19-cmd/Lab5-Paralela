...
void Build_mpi_type(double* a_p, double* b_p, int* n_p, MPI_Datatype* input_mpi_t_p) {
   ...
   array_of_displacements[2] = b_addr - a_addr;
...
void Get_input(int my_rank, int comm_sz, double* a_p, double* b_p, int* n_p) {
   ...
   Build_mpi_type(&input_data.a, &input_data.b, &input_data.n, &input_mpi_t);
   ...
   MPI_Bcast(&input_data, 1, input_mpi_t, 0, MPI_COMM_WORLD);

   // Set the values received from the broadcast
   *a_p = input_data.a;
   *b_p = input_data.b;
   *n_p = input_data.n;
